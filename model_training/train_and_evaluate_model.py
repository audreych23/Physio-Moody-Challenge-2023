...
from data_loading.preload_clinical_data import *
from data_loading.preload_y_data import *
import preview_utilities.evaluation_writer as evaluation_writer

def stratified_k_fold(data_folder, model_folder, graph_folder, verbose, patient_ids, 
    ...
    dummy_x_data = np.zeros(total_num_patients_train)
    y_data = preload_y_data(patient_ids, data_folder)
    ...
    for idx, (train_index, val_index) in enumerate(train_val_split):
        ...
        patient_ids_train = [patient_ids[i] for i in train_index]
        patient_ids_val = [patient_ids[i] for i in val_index]

        print(patient_ids_train)
        print(patient_ids_val)
